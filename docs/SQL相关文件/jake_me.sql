...
-- DROP DATABASE IF EXISTS `jake_me`;

-- CREATE DATABASE `jake_me` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
...
-- columns
-- [mid]        主键，不为空 ,从1开始
-- [fromuid] 	外键，是发送着，是user的外键 	
-- [time] 	是发送私信的时间
-- [content] 	私信的正文
-- [touid] 	是外键，是接受者，是user的外键
...
CREATE TABLE `mail` (
	...
	foreign key(fromuid) references user(uid) on delete cascade on update cascade,
	foreign key(touid)  references user(uid) on delete cascade on update cascade
)AUTO_INCREMENT=1, DEFAULT CHARSET=utf8; 
...
-- [fromuid] 	外键，是关注者，是user的外键 	
-- [touid] 	是外键，是被关注者，是user的外键
-- --------------------------------------------------------------------------------
