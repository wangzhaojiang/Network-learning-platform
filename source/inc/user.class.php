...
class User{
	...
	public function user_signin($mail,$password){
		...
		$key = md5($password);
		$sql = "select * from user where mail='$mail' and password='$key';";
		$sql_error = "用户校验失败";
	...
	public function user_signup($name,$password,$sex,$mail,$school,$sgin){

		if(empty($name) || empty($password) || empty($sex) || empty($mail) ||empty($school))
			return 'false1';
		$permission = 0;
		...
		if(!checkArr($checkArr))
			return 'false2';
		$check_mail_sql = "select mail from user where mail='$mail';";
		$sql_error = "邮箱有重复";
		...
		unset($sql_error);
		$insert_sql = "INSERT INTO user(name,permission,password,sex,mail,school) VALUES ('$name','$permission','$password','$sex','$mail','$school');";
		$sql_error = "增添用户失败";
...
/*	public function user_sendmail($mail){
...
*/
	...
	public function user_changepassword($key,$uid){
		...
		if($result){
			$key = md5($key);
			unset($sql_error);
			$sql_change = "update user set password='$key' where uid='$uid';";
			$sql_error = "修改密码失败";
	...
	public function user_getpermission($uid){
		...
		$sql_error = "获取用户权限失败";
		$sql = "select permission from user where uid='$uid';";
		$result = $this->conn->query($sql,$sql_error);
		if($result != false)
			return  $result;
	...
	//		return 'false1';
		...
		while( $value = current($infoArr) ){
			if( $value != "''" ){
				unset($sql_error);
				$sql_error = "更新用户信息失败";
				$query_str = "UPDATE user set `".key($infoArr)."`=$value"."where uid='$uid';";
